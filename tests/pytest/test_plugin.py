...
class TestRun(TestCase):
    def setUp(self) -> None:
        subprocess_patch = unittest.mock.patch("plugin.subprocess")
    ...
    def test_options(self) -> None:
        test_args = (
            {
                ...
                "kubeconform_command": [plugin.KUBECONFORM_BIN],
            },
            {
        ...
        for arg in test_args:
            with self.subTest(arg=arg):
                ...
                return_code = plugin.main(argv=arg["plugin_args"])
                self.assertEqual(return_code, 0)
                self.subprocess_mock.run.assert_has_calls(calls)
                self.assertEqual(self.subprocess_mock.run.call_count, 2)

    @unittest.mock.patch("plugin.HELM_DEBUG", "true")
    def test_helm_debug(self) -> None:
        calls = [
            unittest.mock.call(
                [plugin.HELM_BIN, "template", "--debug", "chart"],
                stdout=unittest.mock.ANY,
                check=True,
            ),
            unittest.mock.call(
                [plugin.KUBECONFORM_BIN, "-debug"],
                input=unittest.mock.ANY,
                stdout=sys.stderr,
                check=True,
            ),
        ...
        with self.assertLogs(level="DEBUG") as context_manager:
            return_code = plugin.main(argv=["chart"])
        ...
        self.assertIn(
            f"DEBUG:root:Running {plugin.KUBECONFORM_BIN} -debug",
            context_manager.output,
        )
        ...
        errors = [
            ...
            },
            {
                "exception": FileNotFoundError(
                    2, os.strerror(2), plugin.KUBECONFORM_BIN
                ),
                "return_code": 2,
            },
        ]
    ...
    def test_chart_files_as_args(self) -> None:
        ...
        for chart in (
            ...
            self.subprocess_mock.run.assert_has_calls(
                [
                    unittest.mock.call(
                        [plugin.HELM_BIN, "template", chart],
                        stdout=unittest.mock.ANY,
                        check=True,
                    ),
                    unittest.mock.call(
    ...
    def test_chart_file_as_args_failure(self) -> None:
        ...
        with self.assertLogs(level="ERROR") as context_manager:
            return_code = plugin.main(
                argv=[str(p) for p in test_paths],
                validate_chart_files=True,
            )
