...
The error should match pattern "*shellspec: calling $2 * $KUBECONFORM_URL/releases/download/v${HELM_PLUGIN_VERSION%-*}/$kubeconformArchive *"
if [ "$2" = curl ]; then
    The error should not include "shellspec: calling wget "
else
    The error should not include "shellspec: calling curl "
fi
The error should match pattern "*shellspec: calling $2 * $KUBECONFORM_URL/releases/download/v${HELM_PLUGIN_VERSION%-*}/CHECKSUMS *"
if [ "$os" = Darwin ]; then
...
if is_windows "$os"; then
    ...
    The error should include "shellspec: calling chmod +x $HELM_PLUGIN_DIR/bin/kubeconform.exe"
else
    ...
    The error should include "shellspec: calling chmod +x $HELM_PLUGIN_DIR/bin/kubeconform"
fi
